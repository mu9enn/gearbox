...
from GearBox.Common.NetWorkFrame import BaseModel
...
train_data = wavelet_datasets['train_data'][:,[1,2,3,4],:,:]
train_data = train_data[:,:,[1,2,3,5],:]
train_labels = wavelet_datasets['train_labels']
test_data = wavelet_datasets['test_data'][:,[1,2,3,4],:,:]
test_data = test_data[:,:,[1,2,3,5],:]
...
#定义模型
model = BaseModel(class_number=class_num, layer_number=train_data.shape[2])

model_trainer = ModelTrain(model=model)

#训练模型并保存
model_dir = '../BaseModels/Seed_42'
if not os.path.exists(model_dir):
    os.makedirs(model_dir)
path = os.path.join(model_dir, 'Bearing_30_2.pth')
train_acc, test_acc, train_loss, test_loss = model_trainer.train_model(datasets, save_path=path)

#保存并可视化
graph_dir = '../BaseGraphs/Seed_42'
if not os.path.exists(graph_dir):
    os.makedirs(graph_dir)
graph_path = os.path.join(graph_dir, 'Bearing_30_2.png')
