import numpy as np
import torch
import os
import random

from GearBox.Common.ModelTrainAndVisiable import ModelTrain
from GearBox.Common.NetWorkFrame import BaseModelFreq

seed = 42
random.seed(seed)
# 设置NumPy的随机种子
np.random.seed(seed)
# 设置操作系统层面的随机数生成器（部分库会用到）
os.environ['PYTHONHASHSEED'] = str(seed)
torch.manual_seed(seed)
# 设置所有GPU的随机种子
torch.cuda.manual_seed(seed)
torch.cuda.manual_seed_all(seed)

# 确保CUDA卷积操作的确定性
torch.backends.cudnn.deterministic = True
# 禁用cuDNN的自动优化（可能会降低速度，但保证确定性）
torch.backends.cudnn.benchmark = False
#加载数据
wavelet_path = "../../../wavelet_dataset/bearing_30_2.npz"
wavelet_datasets = np.load(wavelet_path)
train_data = wavelet_datasets['train_data'][:,:,[1,2,3,5],:]
train_labels = wavelet_datasets['train_labels']
test_data = wavelet_datasets['test_data'][:,:,[1,2,3,5],:]
test_labels = wavelet_datasets['test_labels']
class_num = len(wavelet_datasets['class_names'])
print(class_num)
datasets = {
    "train_data": train_data,
    "train_labels": train_labels,
    "test_data": test_data,
    "test_labels": test_labels
}

#定义模型
model = BaseModelFreq(class_number=class_num)

model_trainer = ModelTrain(model=model)

#训练模型并保存
model_dir = '../SavedModels_freq/Seed_42'
if not os.path.exists(model_dir):
    os.makedirs(model_dir)
path = os.path.join(model_dir, 'Bearing_30_2.pth')
train_acc, test_acc, train_loss, test_loss = model_trainer.train_model(datasets, save_path=path)

#保存并可视化
graph_dir = '../SavedGraphs_freq/Seed_42'
if not os.path.exists(graph_dir):
    os.makedirs(graph_dir)
graph_path = os.path.join(graph_dir, 'Bearing_30_2.png')
model_trainer.visible(train_accuracy=train_acc, test_accuracy=test_acc, train_loss=train_loss, test_loss=test_loss, save_path=graph_path)