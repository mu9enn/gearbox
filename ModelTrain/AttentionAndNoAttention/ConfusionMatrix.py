...
#加载数据并转为张量
wavelet_dataset_path = '../../wavelet_dataset/wavelet_dataset.npz'
sample = np.load(wavelet_dataset_path, allow_pickle=True)
classes = sample['class_names']
test_set = sample['test_data']
test_label = sample['test_labels']
class_num = np.unique(test_label).shape[0]
print(class_num)
print(test_set.shape, test_label.shape)
if isinstance(test_set, np.ndarray):
    test_set = torch.from_numpy(test_set).float().cpu()
    test_label = torch.from_numpy(test_label).long().cpu()

#加载模型
param_dir = 'SavedModels/Seed_49'
param_file = [m for m in os.listdir(param_dir) if m not in ['Attention_of_SixChannels.pth', 'NoAttention_of_SixChannels.pth']]
param_path_attention = [os.path.join(param_dir, f) for f in param_file if 'NoAttention' not in f]
param_path_no_attention = [os.path.join(param_dir, f) for f in param_file if 'NoAttention' in f]
print(param_path_attention)
print(param_path_no_attention)
...
if __name__ == '__main__':
    param_attention = [torch.load(m, map_location='cpu') for m in param_path_attention]
    param_no_attention = [torch.load(m, map_location='cpu') for m in param_path_no_attention]

    bearing_20_0_id = [np.where(classes == f)[0][0] for f in BearingLabel_20_0]
    bearing_30_2_id = [np.where(classes == f)[0][0] for f in BearingLabel_30_2]
    gear_20_0_id = [np.where(classes == f)[0][0] for f in GearLabel_20_0]
    gear_30_2_id = [np.where(classes == f)[0][0] for f in GearLabel_30_2]

    bearing_20_0_idx = np.isin(test_label, bearing_20_0_id)
    bearing_30_2_idx = np.isin(test_label, bearing_30_2_id)
    gear_20_0_idx = np.isin(test_label, gear_20_0_id)
    gear_30_2_idx = np.isin(test_label, gear_30_2_id)

    bearing_20_0_data = test_set[bearing_20_0_idx]
    bearing_30_2_data = test_set[bearing_30_2_idx]
    gear_20_0_data = test_set[gear_20_0_idx]
    gear_30_2_data = test_set[gear_30_2_idx]

    data = [bearing_20_0_data,bearing_30_2_data,gear_20_0_data,gear_30_2_data]

    bearing_20_0_label = test_label[bearing_20_0_idx]
    bearing_30_2_label = test_label[bearing_30_2_idx]
    gear_20_0_label = test_label[gear_20_0_idx]
    gear_30_2_label = test_label[gear_30_2_idx]
    ...
    weight = np.load('./ChannelWeights/ChannelWeights.npz')['weights']
    weights = torch.from_numpy(weight).float()
    with torch.no_grad():
        with tqdm(param_attention, total=len(param_attention)) as pbar:
            for param in pbar:
                file_name = param_path_attention[pbar.n].split('\\')[-1].split('.')[0]
                confusion_save_path = os.path.join(confusion_dir, f"{file_name}.png")
                f1_save_path = os.path.join(f1_dir, f"{file_name}.png")
                model_attention = CNNNetWorkWithAttention(class_number=class_num, ratio=weights).eval()

                pre_label, real_label = f(parameter=param, dataset=data[pbar.n],data_label=label[pbar.n],model=model_attention)
        ...
        with tqdm(param_no_attention, total=len(param_no_attention)) as qbar:
            for param in qbar:
                file_name = param_path_no_attention[qbar.n].split('\\')[-1].split('.')[0]
                confusion_save_path = os.path.join(confusion_dir, f"{file_name}.png")
                f1_save_path = os.path.join(f1_dir, f"{file_name}.png")
                model_no_attention = CNNNetWorkNoAttention(class_number=class_num).eval()
                pre_label, real_label = f(parameter=param, dataset=data[qbar.n], data_label=label[qbar.n],model=model_no_attention)
                precision_recall_fscore_support(real_labels=real_label,
                                                prediction_label=pre_label,
                                                confusion_path=confusion_save_path,
                                                f1_path=f1_save_path,
                                                class_names=label_classes[qbar.n])
