...
#模型加载
model_path_attention = '../../ModelTrain/AttentionAndNoAttention/SavedModels/Seed_56/Bearing_20_0_Attention.pth'
model_path_no_attention = '../../ModelTrain/AttentionAndNoAttention/SavedModels/Seed_56/Bearing_20_0_NoAttention.pth'

weights = np.load('../../ModelTrain/AttentionAndNoAttention/ChannelWeights/ChannelWeights.npz')['weights']

weights = torch.from_numpy(weights).float()
model_attention = CNNNetWorkWithAttention(class_number=class_num)
...
if __name__ == '__main__':
    ...
    #构建bearing_20_0解释数据集和背景数据集,每种故障类型取50个样本
    ##获取每种故障标签
    bearing_20_0_fault_labels = [np.where(wavelet_datasets['class_names'] == label)[0][0] for label in BearingLabel_20_0]
    # print(fault_labels)
    back_label = np.where(wavelet_datasets['class_names'] == 'bearing_normal_20_0')[0][0]
    # print(back_label)
    ...
    explain_samples = []
    for bearing_20_0_label in bearing_20_0_fault_labels:
        mask_train = np.isin(train_labels, bearing_20_0_label)
        mask_test = np.isin(test_labels, bearing_20_0_label)
    ...
    save_path = os.path.join(save_dir, 'Bearing_20_0_ShapValues.npz')
