...
#模型加载
model_path_attention = '../../ModelTrain/AttentionAndNoAttention/SavedModels/Seed_56/Bearing_30_2_Attention.pth'
model_path_no_attention = '../../ModelTrain/AttentionAndNoAttention/SavedModels/Seed_56/Bearing_30_2_NoAttention.pth'

weights = np.load('../../ModelTrain/AttentionAndNoAttention/ChannelWeights/ChannelWeights.npz')['weights']

weights = torch.from_numpy(weights).float()
model_attention = CNNNetWorkWithAttention(class_number=class_num, ratio=weights)
model_no_attention = CNNNetWorkNoAttention(class_number=class_num)

model_attention.load_state_dict(torch.load(model_path_attention, map_location='cpu'))
model_no_attention.load_state_dict(torch.load(model_path_no_attention, map_location='cpu'))
...
#定义超参数
explainable_number_every_fault = 50
...
if __name__ == '__main__':
    ...
    #构建bearing_30_2解释数据集和背景数据集,每种故障类型取50个样本
    ##获取每种故障标签
    bearing_30_2_fault_labels = [np.where(wavelet_datasets['class_names'] == label)[0][0] for label in BearingLabel_30_2]
    # print(fault_labels)
    back_label = np.where(wavelet_datasets['class_names'] == 'bearing_normal_30_2')[0][0]
    # print(back_label)
    ...
    trian_idx = int(background_sample_number*0.8)
    test_idx = background_sample_number-trian_idx

    background_samples_train_mask = np.random.choice(len(back_train), size=trian_idx, replace=False)
    ...
    explain_samples = []
    for bearing_30_2_label in bearing_30_2_fault_labels:
        mask_train = np.isin(train_labels, bearing_30_2_label)
        mask_test = np.isin(test_labels, bearing_30_2_label)
        ...
        explain_samples.append(fault_samples)

    explain_samples = np.concatenate(explain_samples, axis=0)

    shap_values_attention = calculate_shap_values(model_attention, background_samples, explain_samples)
    shap_values_no_attention = calculate_shap_values(model_no_attention, background_samples, explain_samples)

    #保存shap值
    current_dir = os.path.dirname(__file__)
    save_dir = os.path.join(current_dir, 'Seed_56')
    if not os.path.exists(save_dir):
        os.makedirs(save_dir)
    save_path = os.path.join(save_dir, 'Bearing_30_2_ShapValues.npz')
    ...
    shap_dim_meaning = {
        ...
        4: "类别数（数据集总类别数，含故障/正常样本共20类）"
    }
    np.savez(save_path,
             attention = shap_values_attention,
             no_attention = shap_values_no_attention,
